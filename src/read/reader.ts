...
import { Err, type DataType } from '../core/types';
...
export default abstract class Reader implements ErrorPlaceholder {
     ...
     */
    abstract getErrorPlaceholder(error: Err): Array<Record<string, unknown>> | string
    ...
    protected getCMD(): string {
    	...
    	const includedProperties = propertiesInclude
		  .map((property) => `-${property} `)
		  .join('');
    	const excludedProperties = propertiesExclude
		  .map((property) => `--${property} `)
		  .join('');
